...
public class DatabaseController {
	...
	@Log("读取CSV文件")
	@ApiOperation(value = "读取CSV文件")
	@PostMapping(value = "/readCsv")
	@PreAuthorize("@el.check('database:add')")
	public ResponseEntity<Object> readCsvFile(@RequestParam("file") MultipartFile file){
		return new ResponseEntity<>(databaseService.readCsvFile(file), HttpStatus.OK);
	}

	@Log("导入CSV数据到数据库")
	@ApiOperation(value = "导入CSV数据到数据库")
	@PostMapping(value = "/importCsv")
	@PreAuthorize("@el.check('database:add')")
	public ResponseEntity<Object> importCsvToDatabase(
			@RequestParam("file") MultipartFile file,
			@RequestParam("databaseId") String databaseId,
			@RequestParam("tableName") String tableName){
		return new ResponseEntity<>(databaseService.importCsvToDatabase(file, databaseId, tableName), HttpStatus.OK);
	}
}
